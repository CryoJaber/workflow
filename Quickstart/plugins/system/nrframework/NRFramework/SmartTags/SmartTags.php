...
class SmartTags
{
	...
	private function replaceSmartTagsInContent(&$content, $foundSmartTags)
	{
		...
		foreach ($foundSmartTags as $tag)
		{
			...
			if (!$smartTag = $this->getSmartTagClassByName($smartTagNamespace, $smartTagClassName))
			{
				/**
			...
			$this->prepareSmartTagValue($value);
	 ...
	 * @return  void
	 */
	protected function prepareSmartTagValue(&$value)
	{
		// if the value is an array or object, implode it in order to be able to save it and parse it later on.
		$value = is_array($value) || is_object($value) ? implode(',', (array) $value) : $value;
	}
	 ...
	 * @return  mixed
	 */
	private function getSmartTagClassByName($smartTagNamespace, $smartTagClassName)
	{
		...
		// return smart class
		return new $smartTagClass($this->factory, $this->options);
	}
